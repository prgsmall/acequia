...
AcequiaClient.prototype.subscribe = function (evt) {
    msgEmitter.addListener(evt, objCallback(this, "onMessage"));
};

AcequiaClient.prototype.unsubscribe = function (evt) {
    msgEmitter.removeListener(evt, objCallback(this, "onMessage"));
};
...
WebSocketClient.prototype.send = function (message) {
    ...
    logger.debug("%s:%s", this.protocol, this.id);
    this.update();
};
...
TCPClient.prototype.send = function (message) {
    ...
    logger.debug("%s:[%s:%s]", this.protocol, this.ip, this.port);
...
OSCClient.prototype.send = function (message) {
    ...
    var data = [message.from].concat(message.body),
    oscMsg = osc.newOsc(message.name, 's' + message.tt, message.data),
...
AcequiaClients.prototype.createClient = function (type, from, socket) {
    var rinfo, portOut;
    switch (type) {
    case TYP_OSC:
        return new OSCClient(from, rinfo.address, rinfo.port, portOut, socket);
    case TYP_TCP:
        return new TCPClient(from, socket.remoteAddress, socket.remotePort, socket);
    case TYP_WS:
        return new WebSocketClient(from, socket.id, socket);
    default:
...
AcequiaClients.prototype.onMessage = function (type, message, socket) {
    var client, i, clientList, cli;
    switch (message.name) {
    case msg.MSG_CONNECT:
        client = this.createClient(type, message.from, socket);
        try {
            this.add(client);
    ...
    case msg.MSG_GETCLIENTS:
        ...
        for (cli in this.clients) {
            clientList.push(this.clients[cli].name);
        }
