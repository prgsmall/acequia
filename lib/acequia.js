 ...
 */
AcequiaServer.prototype.broadcastUDP = function (oscServer) {
    ...
    try {
        ...
        this.osc_ad = mdns.createAdvertisement(mdns.udp('acequia', 'osc'), oscServer.address().port, {name: "Acequia OSC Server"});
        this.osc_ad.start();
    } catch (e) {
 ...
 */
AcequiaServer.prototype.broadcastTCP = function (tcpServer) {
    if (!this.options.enableDiscovery) {
        return;
    }
    ...
    try {
        var mdns = require("mdns");
        this.tcp_ad = mdns.createAdvertisement(mdns.tcp('acequia'), tcpServer.address().port, {name: "Acequia TCP Server"});
        this.tcp_ad.start();
    } catch (e) {
        logger.error("Error creating mDNS advertisement: " + e.message);
    }
...
AcequiaServer.prototype.createTCPServer = function () {
    ...
    var listeningCallback = objCallback(this, "broadcastTCP"),
